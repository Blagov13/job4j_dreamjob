...
public class Sql2oCandidateRepository implements CandidateRepository {
    ...
    public Candidate save(Candidate candidate) {
        try (var connection = sql2o.open()) {
            var sql = """
                      INSERT INTO summary(name, description, creation_date, city_id, file_id)
    ...
    public boolean deleteById(int id) {
        try (var connection = sql2o.open()) {
            var query = connection.createQuery("DELETE FROM summary WHERE id = :id");
    ...
    public Optional<Candidate> findById(int id) {
        try (var connection = sql2o.open()) {
            var query = connection.createQuery("SELECT * FROM summary WHERE id = :id");
            query.addParameter("id", id);
    ...
    public Collection<Candidate> findAll() {
        try (var connection = sql2o.open()) {
            var query = connection.createQuery("SELECT * FROM summary");
